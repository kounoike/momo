...
#if USE_ZMQ
#include "zmq_sender/zmq_sender.h"
#endif
...
#if USE_ZMQ
  std::unique_ptr<ZMQSender> zmq_sender = nullptr;
  if (args.use_zmq) {
    zmq_sender.reset(new ZMQSender());
    rtc_manager.reset(new RTCManager(
        std::move(rtcm_config), std::move(capturer), zmq_sender.get()));
  }
#endif
